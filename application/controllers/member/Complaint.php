...
class Complaint extends CI_Controller{
    ...
    function index($start=0){
		$limit = 10;
        $data['complaint'] = $this->Complaint_model->get_limit_users_complaint($limit,$start);
		//pagination
		$config['base_url'] = site_url('member/complaint/index');
		$config['total_rows'] = $this->Complaint_model->get_count_users_complaint();
		$config['per_page'] = 10;
        ...
        $data['_view'] = 'member/complaint/index';
...
'updated_at' =>$updated_at,
        ...
        if(isset($id) && $id>0){
            ...
            if(isset($_POST) && count($_POST) > 0){   
                ...
                redirect('member/complaint/index');
            }else{
                $data['_view'] = 'member/complaint/form';
                $this->load->view('layouts/member/body',$data);
            }
        } //save
		else{
	...
	function details($id){
        $data['complaint'] = $this->Complaint_model->get_complaint($id);
		$data['id'] = $id;
        $data['_view'] = 'member/complaint/details';
        $this->load->view('layouts/member/body',$data);
	}
...
$this->db->or_like('updated_at', $key, 'both');
		...
		//pagination
		$config['base_url'] = site_url('member/complaint/search');
		$this->db->reset_query();		
		$this->db->like('id', $key, 'both');
$this->db->or_like('users_id', $key, 'both');
$this->db->or_like('category_id', $key, 'both');
...
$this->db->or_like('updated_at', $key, 'both');
	...
	function export($export_type='CSV'){
	  if($export_type=='CSV'){	
		   ...
		   // get the HTML
			ob_start();
			include(APPPATH.'views/member/complaint/print_template.php');
			$html = ob_get_clean();
